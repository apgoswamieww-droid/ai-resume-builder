...
export function DarkTemplate({ resume }: TemplateProps) {
  const { personalInfo, experiences, educations, skills, projects, certifications } = resume;
  const themeColor = resume.themeColor || "#3b82f6";

  return (
    <div className="w-full text-slate-100 font-sans min-h-[1050px] flex flex-col" style={{ backgroundColor: "#0f172a" }}>
      {/* Dark Header */}
      <div className="p-8 border-b border-slate-800" style={{ background: `linear-gradient(135deg, ${themeColor}22, transparent)` }}>
        <h1 className="text-3xl font-bold tracking-tight text-white">
          {personalInfo?.fullName || "Your Full Name"}
        </h1>
        {personalInfo?.jobTitle && (
          <p className="text-base font-medium mt-1" style={{ color: themeColor }}>
            {personalInfo.jobTitle}
          </p>
        )}
      ...
      <div className="p-8 space-y-6 flex-1">
        ...
        {experiences && experiences.length > 0 && (
          <div>
            ...
            <div className="mt-3 space-y-4">
              {experiences.map((exp) => (
                <div key={exp.id} className="rounded-xl border border-slate-800 p-4" style={{ backgroundColor: "#1e293b" }}>
                  ...
                  {exp.description && (
                    <p className="mt-1 text-xs text-slate-300 leading-relaxed whitespace-pre-line">
                      {exp.description}
                    </p>
                  )}
                </div>
        ...
        {educations && educations.length > 0 && (
          <div>
            ...
            <div className="mt-3 space-y-3">
              {educations.map((edu) => (
                <div key={edu.id} className="rounded-xl border border-slate-800 p-4" style={{ backgroundColor: "#1e293b" }}>
                  <div className="flex items-baseline justify-between">
        ...
        {projects && projects.length > 0 && (
          <div>
            ...
            <div className="mt-3 space-y-3">
              {projects.map((proj) => (
                <div key={proj.id} className="rounded-xl border border-slate-800 p-4" style={{ backgroundColor: "#1e293b" }}>
                  <div className="flex items-baseline justify-between">
                    <h3 className="text-xs font-bold text-white">{proj.title}</h3>
                    {proj.link && (
                      <span className="text-[11px] text-blue-400 underline">{proj.link}</span>
                    )}
                  </div>
                  {proj.technologies && (
          ...
          <div>
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              {certifications.map((cert) => (
                <span key={cert.id} className="text-xs px-2.5 py-1 rounded-md font-medium" style={{ backgroundColor: `${themeColor}20`, color: themeColor, border: `1px solid ${themeColor}40` }}>
                  {cert.name}
                </span>
              ))}
            </div>
